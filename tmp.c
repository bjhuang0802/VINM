...
int main(void) {
   ...
   double density,BoxLength,HalfBoxLength,NegHalfBoxLength,all;
...
/*   
       for(j=0;j<1;j++){
             //printf("j=%3d, CalZ=%12.6f\n",j,CalT(0,j));
	   for(alpha=0;alpha<4;alpha++){
		 gamma= *(*(*(ato+2)+alpha)+j);
                 //printf("alpha=%3d, mu=%3d, CalDT=%12.6f\n",alpha,mu,CalDT(0,j,gamma,mu));
                 //printf("alpha=%3d, mu=%3d, CalDZ=",alpha,mu);
                 //printf("%12.6f %12.6f %12.6f\n",CalDZ(1,0,j,gamma,mu),CalDZ(2,0,j,gamma,mu),CalDZ(3,0,j,gamma,mu));
	   for(beta=0;beta<4;beta++){
	       for(mu=0;mu<3;mu++){
	       for(nu=0;nu<3;nu++){
		   delta= *(*(*(ato+2)+beta)+j);
		   printf("alpha=%3ld, mu=%3ld,beta=%3ld, nu=%3ld, CalDDT=%12.6f\n",alpha,mu,beta,nu,CalDDT(0,j,gamma,mu,delta,nu));
		   //printf("%3ld, %3ld, %3ld, %3ld, CalDDZ=%12.6f\n",alpha+1,mu+1,beta+1,nu+1,CalDDZ(3,0,j,gamma,mu,delta,nu));
	       }
	   }
	       }
	   }
...
//	       printf("%5ld%5ld%16.5e\n",k,l,*(*(H+k)+l));
//	   }
//    }

       tred2(H,NMolecules*36,Lambda,e);
       tqli(Lambda,e,NMolecules*36,H);

       for(j=0;j<NMolecules*36;j++) {
...
void SetHessianIA2(long NMolecules,
   double ***R,double **H) {
...
void SetHessianIA3(long NMolecules,
   double ***R,double **H) {
...
void SetHessianIR(long NMolecules,
   double ***R,double **H) {
...
void SetHessianLJ14(long NMolecules,
   double ***R,double **H) {
...
void SetHessianCE(long NMolecules,
   double ***R,double **H) {

   long k,gamma,mu,j,delta,beta,nu,i;
   double riajb,Riajb[3];
     ...
     for(gamma=0;gamma<3;gamma++) {
       for(delta=0;delta<3;delta++) {
         for(j=0;j<NMolecules;j++) {
           if(j==k) continue;
           for(beta=0;beta<3;beta++) {
             riajb=GetR(*(*(R+k)+gamma),*(*(R+j)+beta),Riajb);
             for(i=0;i<3;i++)
               Riajb[i]=Riajb[i]/riajb;
             for(mu=0;mu<3;mu++){
               for(nu=0;nu<3;nu++) {
                 *(*(H+k*9+gamma*3+mu+1)+k*9+delta*3+nu+1) +=
                   (DDPhiC(Charges[gamma],Charges[beta],riajb)-
                   DPhiC(Charges[gamma],Charges[beta],riajb)/riajb)*
                   Riajb[mu]*Riajb[nu]*Dt(gamma,delta)+
                   DPhiC(Charges[gamma],Charges[beta],riajb)*
                   Dt(mu,nu)*Dt(gamma,delta)/riajb;
               }
             }
           }
         }
...
void SetHessianCNE(long NMolecules,
   double ***R,double **H) {
     ...
     for(l=k+1;l<NMolecules;l++) {
       for(gamma=0;gamma<3;gamma++) {
         for(delta=0;delta<3;delta++) {
           ...
           for(mu=0;mu<3;mu++) {
             for(nu=0;nu<3;nu++) {
               *(*(H+k*9+gamma*3+mu+1)+l*9+delta*3+nu+1) +=
                 -(DDPhiC(Charges[gamma],Charges[delta],riajb)-
                 DPhiC(Charges[gamma],Charges[delta],riajb)/riajb)*
                 Riajb[mu]*Riajb[nu]-
                 DPhiC(Charges[gamma],Charges[delta],riajb)*
                 Dt(mu,nu)/riajb;
             }
           }
         }
